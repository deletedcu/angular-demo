...
<!--Hook: Has anyone here tried fixing a car or complicated appliance? After two hours, you get the piece back in place or the wheel on straight. Then what do you do? (Personally, I back up and cross my fingers that it works.) The idea behind test-driven development is to gain a higher confidence that before we flip that switch, the app we're building does what the customer wants.  
...
* For those of you who feel negatively toward testing, why is that? What did you or would you do instead?
* For those of you who feel positively toward testing, why is that? What problems did it solve?
 ...
 * **It's too much** - You can test just fine using the console.
 * **App complexity** - The app is too simple to require testing.
* Pros
 * **Bug detection** - It quickly identifies unanticipated errors.
 * **Code Quality** - It can create standards for our code before writing it.
...
**End-to-end tests** explore the application as users experience it, testing for moments when users take an action like visiting a page, clicking a link, logging in, etc.
  ...
  * End-to-end tests have a much wider focus. You'd use end-to-end testing to make sure a sign-in form works or that a user who doesn't have admin privileges can see one page, while a user who does have admin privileges can see another.
...
We are going to be creating something similar to the above example. However, we'll be writing a spec for a `DogComponent`.
...
If you look in the terminal tab that is running `ng test`, you'll see some red text. This means we're failing, which is a sign that our test is working.
...
Add another expectation to the dog that "allows the writing of a hunger level" and follow the same pattern as with the earlier expectation. This test should set the `hungerLevel` property in the component to `5` and then check that it's equal to `5` after it is set.  
...
Imagine we want the eat method to behave differently in different contexts. For example, if the dog is not hungry and has a `hunger_level` of `0`, we don't want the eat method to continue reducing. In order to set up different scenarios or contexts in our specifications, we can use a different `describe` blocks.
...
Write the code to pass the above specifications. 
